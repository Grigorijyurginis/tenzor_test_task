...
Необходимо импортировать данные по организациям и пользователям в SQL таблицу из приложенного к заданию json файла. У компании есть офисы (type = 1) в нескольких городах, в каждом из них есть отделы (type = 2), где работают сотрудники (type = 3). Необходимо реализовать выборку всех сотрудников по указанному идентификатору сотрудника.
...
На вход утилите подается идентификатор сотрудника = 3 На выходе получается список сотрудников в офисе Санкт-Петербурга: Иванов, Сидоров, Петров.
...
- a. документа описывающего текущее решение

- b. документа описывающего текущую сложность выборки SQL и перспективы оптимизации;

- c. исходных файлов (которые можно выполнить и посмотреть на работу программы);

- d. сторонних библиотек, которые нужны для работы.
